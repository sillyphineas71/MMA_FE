...
export default function OwnerSubPitchEditScreen({ navigation, route }) {
    ...
    const handleAddBlock = () => {
        if (!newStart || !newEnd || !newPrice) {
            Alert.alert("Thiếu thông tin", "Vui lòng nhập Giờ bắt đầu, Giờ kết thúc và Giá.");
            return;
        }
        setBlocks([...blocks, { start: newStart, end: newEnd, price: parseInt(newPrice) || 0 }]);
        setNewStart("");
        setNewEnd("");
        setNewPrice("");
    };
    ...
    const handleUpdate = async () => {
        ...
        try {
            await apiPut(`/owner/sub-pitches/${subPitch._id}`, payload);
            Alert.alert("✅ Thành công", "Đã cập nhật sân con!");
            navigation.goBack();
        } catch (err) {
            console.error("L_ERROR_RESPONSE_DATA:", JSON.stringify(err.response?.data, null, 2));
            const errorMessage = err.response?.data?.message || err.message || "Không thể cập nhật sân con.";
    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            >
                ...
                <TextInput style={styles.input} placeholder="Tên sân (Sân 1, Sân 2...)" value={name} onChangeText={setName} placeholderTextColor="#9CA3AF" />
                ...
                <View style={styles.typeContainer}>
                    {PITCH_TYPES.map((pitchType) => (
                        <TouchableOpacity
                            key={pitchType}
                            style={[styles.typeButton, type === pitchType && styles.typeButtonActive]}
                            onPress={() => setType(pitchType)}
                        >
                            <Text style={[styles.typeButtonText, type === pitchType && styles.typeButtonTextActive]}>
                                {pitchType}
                            </Text>
                        </TouchableOpacity>
                    ))}
                ...
                <View style={styles.addBlockContainer}>
                    <View style={styles.timeRow}>
                        <TouchableOpacity style={styles.timeInput} onPress={() => setShowTimePicker('start')}>
                            <Text style={[styles.timePickerText, !newStart && styles.timePickerPlaceholder]}>
                                {newStart || "Giờ bắt đầu"}
                            </Text>
                        </TouchableOpacity>
                        <TouchableOpacity style={styles.timeInput} onPress={() => setShowTimePicker('end')}>
                            <Text style={[styles.timePickerText, !newEnd && styles.timePickerPlaceholder]}>
                                {newEnd || "Giờ kết thúc"}
                            </Text>
                        </TouchableOpacity>
                    </View>
                    ...
                    <TextInput style={styles.input} placeholder="Giá (VD: 200000)" value={newPrice} onChangeText={setNewPrice} keyboardType="numeric" placeholderTextColor="#9CA3AF" />
                    <TouchableOpacity style={styles.buttonOutline} onPress={handleAddBlock}>
                ...
                {blocks.map((block, index) => (
                    <View key={index} style={styles.blockItem}>
                        <View>
                            <Text style={styles.blockTime}>Khung: {block.start} - {block.end}</Text>
                            <Text style={styles.blockPrice}>Giá: {block.price.toLocaleString("vi-VN")} VNĐ</Text>
                        </View>
