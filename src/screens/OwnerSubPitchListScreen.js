...
export default function OwnerSubPitchListScreen({ navigation, route }) {
  ...
  const fetchSubPitches = async () => {
    try {
      const data = await apiGet(`/api/owner/venues/${venueId}/sub-pitches`);
      setSubPitches(data || []);
    } catch (err) {
  ...
  const handleDelete = (subPitch) => {
    Alert.alert("Xác nhận xoá", `Bạn có chắc muốn xoá "${subPitch.name}"?`, [
      { text: "Huỷ" },
      {
        ...
        onPress: async () => {
          try {
            await apiDelete(`/api/owner/sub-pitches/${subPitch._id}`);
  ...
  const createSubPitch = () =>
    navigation.navigate("OwnerSubPitchCreate", { venueId });
  const editSubPitch = (subPitch) =>
    navigation.navigate("OwnerSubPitchEdit", { subPitch });
    ...
    <View style={styles.card}>
      <View style={styles.cardContent}>
        ...
        <Text
          style={[
            styles.status,
            { color: item.active ? "#40B800" : "#EF4444" },
          ]}
        >
        ...
        <Text style={styles.blocks}>
          ...
          {item.bookableBlocks?.length || 0} khung giờ
        </Text>

        <View style={styles.actions}>
          <TouchableOpacity
            style={styles.actionBtn}
            onPress={() => viewReviews(item)}
          >
