...
export default function OwnerReviewListScreen({ navigation, route }) {
    ...
    const fetchReviews = async () => {
        try {
            const data = await apiGet(`/owner/sub-pitches/${subPitchId}/reviews`);
            setReviews(data || []);
        } catch (err) {
    ...
    if (loading) {
        return (
            <View style={styles.loadingContainer}>
                ...
                <Text style={{ marginTop: 10, color: "#40B800" }}>Đang tải đánh giá...</Text>
            </View>
        );
    }
    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            <FlatList
                ...
                refreshControl={
                    <RefreshControl refreshing={refreshing} onRefresh={onRefresh} colors={["#40B800"]} />
                }
            />
        </SafeAreaView>
    );
