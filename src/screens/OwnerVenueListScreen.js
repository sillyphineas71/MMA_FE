...
import {
    ...
    TouchableOpacity,
    StyleSheet,
    SafeAreaView, // Đảm bảo đã import
    RefreshControl,
    Alert,
    TextInput,
} from "react-native";
...
export default function OwnerVenueListScreen({ navigation }) {
    ...
    if (Constants.manifest2?.extra?.expoGo?.debuggerHost) {
        ...
        try {
            ...
            console.error("Không thể phân tích experienceUrl:", Constants.experienceUrl, e);
        }
    }
    if (!host) {
        console.warn("Không thể tự động nhận diện IP. Đang dùng 'localhost' làm dự phòng.");
        host = 'localhost';
    }
                ...
                <View style={styles.cardContent}>
                    ...
                    <View style={styles.actions}>
                        ...
                        <TouchableOpacity style={styles.actionBtn} onPress={() => toggleStatus(item)}>
                            <Feather name={item.status === "active" ? "eye-off" : "eye"} size={18} color="#EF4444" />
        ...
        <SafeAreaView style={styles.container}>
            ...
            <View style={styles.filterContainer}>
                ...
                <View style={styles.filterButtonsContainer}>
                    ...
                    >
                        <Text style={[styles.filterButtonText, activeFilter === "active" && styles.activeFilterButtonText]}>
                            Đang hoạt động
                        </Text>
                    </TouchableOpacity>

                    <TouchableOpacity
                        style={[styles.filterButton, activeFilter === "hidden" && styles.activeFilterButton]}
                        onPress={() => setActiveFilter("hidden")}
                    >
                        <Text style={[styles.filterButtonText, activeFilter === "hidden" && styles.activeFilterButtonText]}>
                            Đang ẩn
                        </Text>
                    </TouchableOpacity>
                </View>
