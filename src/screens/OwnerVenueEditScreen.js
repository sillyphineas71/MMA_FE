...
export default function OwnerVenueEditScreen({ navigation, route }) {
    ...
    useEffect(() => {
        if (venue) {
            ...
            const venueLocation = venue.location?.coordinates && venue.location.coordinates.length === 2
                ? venue.location
        ...
        try {
            ...
            if (hasNewImages) {
                ...
                newImages.forEach((uri) => {
                    ...
                    formData.append("images", { // <--- Dùng "images[]"
                        uri,
                ...
                response = await apiPutForm(`/owner/venues/${venue._id}`, formData);
                ...
                response = await apiPut(`/owner/venues/${venue._id}`, payload);
            }
            ...
            const errorMessage = err.message || "Không thể cập nhật sân. Vui lòng thử lại.";
